...
public class StudyQueryTestRepository {
    ...
    public List<Study> findStudiesWithCondPart1(Predicate whereClause, Pageable pageable) {
        QStudy study = QStudy.study;
    ...
    public PagedResponse<StudyQueryResponse> findStudiesWithCondPart2(Predicate whereClause, Pageable pageable, List<Study> studies) {
        QStudy study = QStudy.study;
    ...
    public List<Study> findAllStudiesWithNoCond(StudySearchCond searchCond, Pageable pageable) {
        BooleanExpression expression = searchCond != null
                ? titlesLike(QStudy.study, searchCond.getTitles())
                : Expressions.asBoolean(true).isTrue();

        return findStudiesWithCondPart1(expression, pageable);
    }
    ...
    public PagedResponse<StudyQueryResponse> findStudyWishList2(List<String> tags, List<Long> zoneIds, Pageable pageable) {
        ...
        Long totalCount = queryFactory
                ...
                .from(study)
                .join(study.studyTags, studyTag).on(tagsInSub(study, tags))
                .join(studyTag.tag, tag)
                .join(study.studyZones, studyZone).on(zoneIdsInSub(study, zoneIds))
    ...
    private BooleanExpression tagsInSub(QStudy study, List<String> tags) {
    ...
    private BooleanExpression zoneIdsInSub(QStudy study, List<Long> zoneIds) {
        if (zoneIds == null || zoneIds.isEmpty()) {
            return Expressions.asBoolean(true).isTrue();
        }
